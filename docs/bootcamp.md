...
| 0.3 | Include at least 2 Open-source Helm subcharts (not a fork), 1 should provide the stateful component; embedded as default and BYOas opt-in | Show the subchart declared with a proper conditional. Install with BYO (show no stateful component pod, app is using the external instance). Set the Helm value to enable the embedded component and show the stateful component pod Running. |  |
...
| 1.4 | Notifications \- enable email notifications when new releases are promoted to the Stable channel | Only when promoted to stable, you should receive an email to your @[replicated.com](http://replicated.com) address |  |
...
We will use Enterprise Portal v2. See [docs](https://docs.google.com/document/d/1ls9AkRXPnpc89xk7D2r5aNGTZHe4zDTYgcf7KUQACiQ/edit).
...
| \# | Task | Acceptance Criteria | Notes |
| :---- | :---- | :---- | :---- |
| 7.1 | Create notifications | you can show some email and webhook notifications that are triggered on various account activity |  |
